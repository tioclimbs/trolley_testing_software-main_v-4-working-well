...
bool TFMPlus::getData()
{
  ...
  while ((*pStream).available() > TFMP_FRAME_SIZE)
    (*pStream).read();
  ...
  for (uint8_t i = 0; i < (TFMP_FRAME_SIZE - 1); i++)
    chkSum += frame[i];
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  data.dist = frame[2] + (frame[3] << 8);
  data.flux = frame[4] + (frame[5] << 8);
  data.temp = frame[6] + (frame[7] << 8);
  // Convert temp code to degrees Celsius.
  data.temp = (data.temp >> 3) - 256;
...
void TFMPlus::printStatus()
{
  ...
  if (status == TFMP_READY)
    Serial.print("READY");
  else if (status == TFMP_SERIAL)
    Serial.print("SERIAL");
  else if (status == TFMP_HEADER)
    Serial.print("HEADER");
  else if (status == TFMP_CHECKSUM)
    Serial.print("CHECKSUM");
  else if (status == TFMP_TIMEOUT)
    Serial.print("TIMEOUT");
  else if (status == TFMP_PASS)
    Serial.print("PASS");
  else if (status == TFMP_FAIL)
    Serial.print("FAIL");
  else if (status == TFMP_I2CREAD)
    Serial.print("I2C-READ");
  else if (status == TFMP_I2CWRITE)
    Serial.print("I2C-WRITE");
  else if (status == TFMP_I2CLENGTH)
    Serial.print("I2C-LENGTH");
  else if (status == TFMP_WEAK)
    Serial.print("Signal weak");
  else if (status == TFMP_STRONG)
    Serial.print("Signal saturation");
  else if (status == TFMP_FLOOD)
    Serial.print("Ambient light saturation");
  else
    Serial.print("OTHER");
  Serial.println();
}
...
void TFMPlus::printFWValues()
{
  sendCommand(GET_FIRMWARE_VERSION, 0);
